...
def who_shall_I_Take_Care_of(lista_pac: list[cPacient],init: int,end: int) -> cPacient:
    ...
    if (end - init == 0):                                       ##tengo un solo paciente en la lista
        ...
        coef1 = lista_pac[init-1].colour / lista_pac[init-1].timeremaining()
        coef2 = lista_pac[end-1].colour / lista_pac[end-1].timeremaining()
        if (coef1 >= coef2):
            return lista_pac[init-1]
        else:
        ...
        coef1 = pac_aux1.colour / pac_aux1.timeremaining()
        coef2 = pac_aux2.colour / pac_aux2.timeremaining()
        if (coef1 >= coef2):
            return pac_aux1
        else:
            return pac_aux2